...
int portno, sockfd_r, direct_nodes;
int all_nodes_count, direct_nodes;
...
int main(int argc, char **argv)
{
  ...
  char buff[MAXLINE], source_switch[20], *node_holder1, *node_holder2, node1[MAXLINE], node2[MAXLINE], comment_checker[10], all_nodes[16][MAXLINE], *context, temp_msg[MAXLINE];
...
/*  if ( (childpid = fork()) != 0 )           				//Parent will call function which will run in loop to send data every 30 seconds...
  ...
  {
 	 fprintf(stderr, "Unable to craate pthread for func route send \n",t);
                        exit(1);
  }
...
//			printf("\nTime to send routing update to the neighbors...\n");
 ...
 void * route_recv (char d_connect_node[])
  {	
	...
	fd_set rset;
	char r_buff[MAXLINE], cli_address[MAXLINE], temp_buff[MAXLINE], buff[MAXLINE], *up_route_table;
	FD_ZERO(&rset);
	...
	memcpy(&routecopy_17, &route17, sizeof(route17));
        for (i= 0; i< all_nodes_count; i++)
        {
                ...
                for (j=0; j< all_nodes_count ; j++)
                {
                        ...
                        if((strcmp (routecopy_17[i].dest, route18[j].dest) == 0) && routecopy_17[i].cost !=0 && route18[j].cost !=0)
                        {
                                printf("Debug: Node match... Verify cost...\n");
                                if(routecopy_17[i].cost <= route18[j].cost)
                                        break;
				for(k=0; k< all_nodes_count; k++)
				{
					if(strcmp (routecopy_17[k].dest, route18[j].hop) == 0)
					{
					 	if(routecopy_17[i].cost > (route18[j].cost + routecopy_17[k].cost))
						{
                                       			printf("Debug: Updating route... of %s with cost %d and hop %s\n", route17[i].dest, (route18[j].cost+route17[k].cost), route18[j].hop);
							route17[i].cost = route18[j].cost+route17[k].cost;
...
//	strcpy(routing_table,updated_routing_table);
	strcpy(return_table,updated_routing_table);
        ...
        printf("Value of the routing table in string format is:%s",routing_table);   
	return(return_table);	
  }
