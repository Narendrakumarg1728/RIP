...
int main(int argc, char **argv)
{
  ...
  char buff[MAXLINE], source_switch[20], temp_node[20], *node_holder1, *node_holder2, node1[MAXLINE], node2[MAXLINE], comment_checker[10], all_nodes[16][MAXLINE], *context, temp_msg[MAXLINE];
...
//	strcpy(th_direct[t].r_table, routing_table);
	...
	printf("Direct connect node is:%s\n",temp_node);
  	...
  	if (pthread_create(&tid[t], NULL, &route_recv, (void *) temp_node))
        	{
...
//			printf("Return value of route_compare is:%d\n",r);
		...
		else
		{
			...
			printf("Not received data from the neighbor:%s with in 180 secs \nModify the routing table to reflect infinit (16)\n",d_connect_node);
		        up_route_table = route_modifier(d_connect_node,16);
  ...
  char * route_modifier (char node[MAXLINE], int value)
  {
	char route_update[MAXLINE], *return_table = malloc(MAXLINE+1), updated_routing_table[MAXLINE], cost[20];
