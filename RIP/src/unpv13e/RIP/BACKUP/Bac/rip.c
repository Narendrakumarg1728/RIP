...
void route_compare( char c_buff[]);
...
//                	err_quit("inet_pton error for %s\n", m_recv.phy_ip);
		...
		if (FD_ISSET (sockfd_r, &rset))
		{
			...
			printf("Calling route compare fumction...\n");
			route_compare(r_buff);
  ...
  void route_compare( char rec_r_table[])
  {
	char temp_r_table[MAXLINE], temp_rec_r_table[MAXLINE];
	struct route route17[all_nodes_count], route18[all_nodes_count];
	int i;
	
	strcpy(temp_r_table, routing_table);
	strcpy(temp_rec_r_table, rec_r_table);
	
        route17=string_to_struct_route(temp_r_table);
	route18=string_to_struct_route(temp_rec_r_table);

	printf("Value of routing table from struct...\n");
	printf("Dest\t\t\thop\t\t\thop\t\n");
	for (i = 0; i < all_nodes_count; i++)
        {
		printf("%s\t%d\t%s\t\n",route17[i].dest, route17[i].cost, route17[i].hop);
	}
	printf("Value of newly received routing table from struct...\n");
	printf("Dest\t\t\thop\t\t\thop\t\n");
	for (i = 0; i < all_nodes_count; i++)
        {
  ...
  struct route string_to_struct_route(char r_table[MAXLINE])
  {
	...
	for (i = 0; i< all_nodes_count; i++)
	{
		bzero(route16[i], sizeof(route16));
	}
	for (i = 0; i < all_nodes_count; i++)
	{
		n_holder1 = strtok_r(r_table, " ", &context);
		if (n_holder1 == NULL)
			continue;
		strcpy(route16[i].dest, n_holder1);
		
		n_holder2 = strtok_r(NULL, " ", &context);
                if (n_holder2 == NULL)
                        continue;
                strcpy(cost, n_holder2);
		route16[i].cost = atoi(cost);
                n_holder3 = strtok_r(NULL, " ", &context);
                if (n_holder3 == NULL)
                        continue;
                strcpy(route16[i].hop,n_holder3);
	}
	return route16[];
  }
