...
//                	err_quit("inet_pton error for %s\n", m_recv.phy_ip);
  ...
  int route_compare( char rec_r_table[MAXLINE])
  {
	...
	printf("Current Routing table...:%s\n",temp_r_table);
	printf("Before 2nd strcpy...\n");
	strcpy(temp_rec_r_table, rec_r_table);
	printf("Routing table from neighbor...:%s\n",temp_r_table);
	printf("All nodes count is:%d",all_nodes_count);
        for (i = 0; i < all_nodes_count; i++)
        {
		printf("Inside 1st for\n");
                n_holder1 = strtok_r(temp_r_table, " ", &context);
                if (n_holder1 == NULL)
                        continue;
                strcpy(route17[i].dest, n_holder1);
		printf("dest:%s\n",route17[i].dest);
                n_holder2 = strtok_r(NULL, " ", &context);
                if (n_holder2 == NULL)
                        continue;
		...
		printf("Inside 2nd for\n");
                n_holder1 = strtok_r(temp_rec_r_table, " ", &context);
                if (n_holder1 == NULL)
                        continue;
                strcpy(route18[i].dest, n_holder1);
		printf("dest:%s\n",route18[i].dest);
