...
//			printf("\nTime to send routing update to the neighbors...\n");
	...
	if (select(0, NULL, NULL, NULL, &tv) < 0) 
		err_sys("select error in route_send func\n");
	}
	return(-1);
  }
 ...
 void * route_recv (char d_connect_node[])
  {	
	...
	fd_set rset;
	char r_buff[MAXLINE], cli_address[MAXLINE], temp_buff[MAXLINE], buff[MAXLINE], *updated_route;
	FD_ZERO(&rset);
...
//                	err_quit("inet_pton error for %s\n", m_recv.phy_ip);
		...
		if ( (rv =select(max_fd, &rset , NULL, NULL, &tv)) < 0)
                	err_sys("select error in route_recv\n");
		...
		if (FD_ISSET (sockfd_r, &rset))
		{
			...
			*updated_route = route_compare(temp_buff);
			printf("Return value which is the updated route from the route_compare is %s\n",updated_route);

//			Pthread_mutex_lock(&routing_mutex);
			strcpy(routing_table,updated_route);
//			Pthread_mutex_unlock(&routing_mutex);
  ...
  char route_compare( char rec_r_table[MAXLINE])
  {
	...
	char  *n_holder1, *n_holder2, *n_holder3, *context, cost[20], cost_update[20];
	const char * u_routing_table_return;
...
//Comparing route table with the received data...
	...
	for (i = 0; i < all_nodes_count; i++)
		...
		sprintf(cost_update, "%d", route17[i].cost);
		strcat(updated_routing_table, cost_update);
		strcat(updated_routing_table, "\t");
		strcat(updated_routing_table, route17[i].hop);
		strcat(updated_routing_table, "\n");
	}
	
	strcpy(u_routing_table_return, updated_routing_table);
	printf("Value of the routing table in string format is:%s",updated_routing_table); 
	printf("Value of the routing table as const string format is:%s",u_routing_table_return); 
